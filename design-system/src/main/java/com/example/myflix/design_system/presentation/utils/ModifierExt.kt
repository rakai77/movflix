...
fun Modifier.carouselTransition(
    ...
    graphicsLayer {
        ...
        val maxRotateDegree = 10f
        val rotationDegree =
            if (isLastPage && pagerState.currentPage == pagerState.pageCount - 1) {
                0f
            } else {
                maxRotateDegree * (if (pageOffset > 0) -1 else 1) * absPageOffset.coerceIn(0f, 1f)
            }
        rotationZ = rotationDegree
        translationX = pageOffset * (size.width / 1.2f)
//        translationY =
//            (if (pageOffset > 0) 1 else -1) * (pageOffset * ((MathUtils.calculateRotateHeight(
//                size.width,
//                size.height,
//                rotationDegree
//            ) - size.height / 2)))
